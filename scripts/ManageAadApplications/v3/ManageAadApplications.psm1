...
function Update-AadApplication {
    ...
    $app = Get-AzADApplication -ObjectId $ObjectId -ErrorAction Continue
    ...
    if (!$app) {
        ...
        if ($null -eq $AppIdUri -or $AppIdUri -eq "") {
            Write-Host "AppIdUri is null or empty so use AppIdUri from the AD $($app.IdentifierUris[0])"
            $AppIdUri = $app.IdentifierUris[0]
        ...
        if ($null -eq $HomePageUrl -or $HomePageUrl -eq "") {
            Write-Host "HomePageUrl is null or empty so use HomePage from the AD $($app.HomePage) because Microsoft Update cmdlet won't allow empty Homepage"
        ...
        Set-AzureADApplication -ObjectId $app.ObjectId -Oauth2AllowImplicitFlow $Oauth2AllowImplicitFlow -RequiredResourceAccess $requiredResourceAccess -AppRoles $appRoles
