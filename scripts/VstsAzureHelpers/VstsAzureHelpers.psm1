...
function Initialize-Module {
    ...
    #region check if module is locally available
    $targetAzurePs = Get-AzureRMVersion -AzureRMModuleName $Name -RequiredVersion $RequiredVersion
    $result = [string]$($targetAzurePs) -match '[2]+\.[1]+\.[0]|[3]+\.[8]+\.[0]|[4]+\.[2]+\.[1]|[5]+\.[1]+\.[1]|[6]+\.[7]+\.[0]'
    if ($result) {
        Write-Verbose -Message ('Module {0} with Version {1} is locally available in AzureRM version {2}' -f $Name, $RequiredVersion, $targetAzurePs)
        Write-Verbose "Add local AzureRM PowerShell modules path to the PSModulePath Environment variable"
        $modulePath = ('c:\Modules\AzureRM_{0}' -f $targetAzurePs)
        ...
        if (!($module)) {
            ...
            # Check if there are multiple locations where the Module version is located.
